...
def main():
    ...
    train_iter = get_arranger(opts["train"], [source_dict, target_dict], shuffling_corpus=opts["shuffle_training_data"], shuflling_buckets=opts["shuffle_training_data"], sort_prior=sort_prior, batch_size=opts["batch_size"], maxibatch_size=20, max_len=opts["max_len"], min_len=2, one_len=opts["max_len"])
    dev_iter = get_arranger(opts["dev"], [source_dict, target_dict], shuffling_corpus=False, shuflling_buckets=False, sort_prior=[0], batch_size=opts["valid_batch_size"], maxibatch_size=-1, max_len=utils.Constants.MAX_V, min_len=0, one_len=opts["max_len"])
    # 3. about model & trainer
    mm = s2sModel(opts, source_dict, target_dict)
