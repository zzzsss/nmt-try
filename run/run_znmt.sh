...
python3.5 ${py_args} ${zmt}/znmt/test.py -v --report_freq 125 -t ${datadir}/test.final.{${src},${trg}.restore} -d ${rundir}/{"src","trg"}.v -m ${rundir}/zbest.model -n 1.0 -o ${output} --dynet-devices ${_dy_device} --beam_size ${test_beam_size} ${extras}
ZMT=${zmt} bash ${zmt}/znmt/scripts/restore.sh <${output} | perl ${zmt}/znmt/scripts/multi-bleu.perl ${datadir}/test.final.${trg}.restore
